...
func BenchmarkConfigProtoUnmarshal(b *testing.B) {
	...
	for i := 0; i < 10000; i++ {
		data, err := github_com_maditya_protobuf_proto.Marshal(NewPopulatedConfig(popr, false))
		if err != nil {
			panic(err)
		}
		datas[i] = data
...
func BenchmarkRealmConfigProtoUnmarshal(b *testing.B) {
	...
	datas := make([][]byte, 10000)
	for i := 0; i < 10000; i++ {
		data, err := github_com_maditya_protobuf_proto.Marshal(NewPopulatedRealmConfig(popr, false))
		if err != nil {
			panic(err)
		}
		datas[i] = data
...
func BenchmarkRealmConfigSize(b *testing.B) {
	popr := math_rand.New(math_rand.NewSource(616))
	total := 0
	pops := make([]*RealmConfig, 1000)
	for i := 0; i < 1000; i++ {
		pops[i] = NewPopulatedRealmConfig(popr, false)
	}
	b.ResetTimer()
