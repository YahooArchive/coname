...
func TestTimestampMarshalTo(t *testing.T) {
	seed := time.Now().UnixNano()
	popr := math_rand.New(math_rand.NewSource(seed))
	p := NewPopulatedTimestamp(popr, false)
	size := p.Size()
...
func BenchmarkTimestampProtoMarshal(b *testing.B) {
	popr := math_rand.New(math_rand.NewSource(616))
	total := 0
	pops := make([]*Timestamp, 10000)
	for i := 0; i < 10000; i++ {
		pops[i] = NewPopulatedTimestamp(popr, false)
	}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkTimestampProtoUnmarshal(b *testing.B) {
	popr := math_rand.New(math_rand.NewSource(616))
	total := 0
	datas := make([][]byte, 10000)
	for i := 0; i < 10000; i++ {
		data, err := github_com_andres_erbsen_protobuf_proto.Marshal(NewPopulatedTimestamp(popr, false))
		if err != nil {
			panic(err)
		}
		datas[i] = data
...
func TestTimestampVerboseEqual(t *testing.T) {
...
func BenchmarkTimestampSize(b *testing.B) {
	popr := math_rand.New(math_rand.NewSource(616))
	total := 0
	pops := make([]*Timestamp, 1000)
	for i := 0; i < 1000; i++ {
		pops[i] = NewPopulatedTimestamp(popr, false)
	}
	b.ResetTimer()
